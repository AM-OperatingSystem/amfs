...
impl Allocator {
    ...
    pub fn prealloc(&self, dgs: &mut [Option<DiskGroup>],n: u8) -> AMResult<Vec<AMPointerGlobal>> {
        ...
        let blks = if exlen==0 { 1 } else { (exlen+(ent_each-1))/ent_each };
        let mut res = dgs[n as usize].as_mut().ok_or(0)?.alloc_many(blks as u64)?;
        loop {
            let exlen = self.0.borrow().extents.len();
            let blks = if exlen==0 { 1 } else { (exlen+(ent_each-1))/ent_each };
            if res.len()>=blks { break; }
            res.append(&mut dgs[n as usize].as_mut().ok_or(0)?.alloc_many((blks-res.len()) as u64)?)
        }
        Ok(res)
    }
...
impl AllocatorObj {
    ...
    fn alloc(&mut self, size: u64) -> Option<u64> {
        ...
        if let Some((a,sa,se)) = exs {
            *self.extents.get_mut(&a).unwrap() = Extent{size: sa, used: true};
            self.extents.insert(a+sa,Extent{size: se-sa, used: false});
            return Some(a);
        }
        None
    }
    ...
    fn mark_used(&mut self, start: u64, size: u64) -> AMResult<()> {
        ...
        if start == c.0 {
            if c.1 == size {
                ...
                ex.used = true;
                ex.size = size;
                self.extents.insert(c.0+size,Extent{size: c.1-size, used: false});
            }
            ...
            ex.size = start-c.0;
            self.extents.insert(start,Extent{size, used: true});
            self.extents.insert(start+size,Extent{size: (c.0+c.1)-(start+size), used: false});
        }
        Ok(())
    }
